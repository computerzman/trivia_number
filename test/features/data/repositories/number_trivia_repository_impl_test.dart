...
group('getConcreteNumberTrivia', (){
  ...
  final NumberTrivia tNumberTrivia = tNumberTriviaModel; // Casting the tNumberTriviaModel in the Entity Type of NumberTrivia
...
test(
  ...
  // arrange
    when(mockNetworkInfo.isConnected).thenAnswer((_) async => true); // we used thenAnswer because we use Future when getting network info
  // act
...
group('device is offline', (){
  setUp((){
      when(mockNetworkInfo.isConnected).thenAnswer((_) async => false); // we used thenAnswer because we use Future when getting network info
  });
...
runTestsOnline( (){
  ...
  () async {
    ...
    expect(result, equals(Right(tNumberTrivia))); // Right(tNumberTrivia) => is the right value of Future<Either<Failure, NumberTrivia>>
...
test(
  ...
  () async {
    ...
    // assert
    verify(mockRemoteDataSource.getConcreteNumberTrivia(tNumber));
    //Finally when we're online and the remote Data Source throws a ServerException, we should convert it to a ServerFailure and return it from the method. 
    // In such case, nothing should be cached locally (thus verifyZeroInteractions).
    verifyZeroInteractions(mockLocalDataSource);
    expect(result, equals(Left(ServerFailure()))); // Left(ServerException()) => is the Left value of Future<Either<Failure, NumberTrivia>>
